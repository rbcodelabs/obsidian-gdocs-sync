...
Click Connect in each numbered test window, then authorize the selected disposable account in normal system Chrome with a temporary profile. The harness attaches through a loopback debugging port and installs CDP request-stage callback capture on the sign-in page before navigation, including HTTP redirect hops; no DevTools or callback copying is needed. Credentials remain in each isolated Geode profile; they are not copied between clients. The upstream token-bearing redirect never reaches the browser or OS protocol handler. Callback origin/path and encoded/final state are checked; rejection shows an inert failure page and stops the stage. After clean completion, only the owned tab's session navigation history is cleared. No tracing, HAR, request logging, or auth screenshots are enabled. Each sign-in has a ten-minute timeout. Resume uses credentials only from those same explicitly created Geode profiles.
