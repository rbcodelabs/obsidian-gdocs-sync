...
- **rename-delete-edit:** synthetic rename/edit and delete/edit branches while two clients are paused; all three clients must retain both conflicts. This stage intentionally leaves those versions unresolved for inspection.
...
Byte checks compare against independently computed synthetic source hashes, not merely agreement between clients. Soak also requires exactly one expected file per completed tick. Every sync rejects pending dependencies, blocked/excluded paths, and unhealthy status; only the explicitly exercised conflict paths are allowed during conflict scenarios. The rename/delete stage's known unresolved paths remain allowed in subsequent stages, while other conflicts fail acceptance.
