...
From v0.10.2, Drive identifying an uploaded binary blob as an image or another content type no longer causes a false integrity failure. Folder identity and JSON history-record MIME checks remain strict, and blob length/SHA-256 verification is unchanged. Metadata errors now identify the mismatched field. If an integrity error persists after updating and reloading the plugin, preserve the local journal and managed remote history; do not delete or reset them to bypass the check. Record the failing field for diagnosis and redact filenames or identifiers before sharing diagnostics publicly.
